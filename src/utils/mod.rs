...
pub fn field_to_u32<F:BaseExt>(f: &F) -> u32 {
    let mut bytes = vec![];
    f.write(&mut bytes).unwrap();
    u32::from_le_bytes(bytes[0..4].try_into().unwrap())
}

pub fn field_to_u64<F:BaseExt>(f: &F) -> u64 {
    let mut bytes = vec![];
    f.write(&mut bytes).unwrap();
...
pub fn cell_to_u32<F:FieldExt>(cell: &AssignedCell<F,F>) -> u32 {
    cell.value().map_or(0,|value| field_to_u32(value))
}
...
macro_rules! customized_circuits {
    ($name:ident, $row:expr, $adv:expr, $fix:expr, $sel:expr, $($item:tt)*) => {
        customized_circuits_expand!($name, $row, $adv, $fix, $sel, $($item)*);
    };
}
...
mod tests {
    ...
    use halo2_proofs::arithmetic::FieldExt;
    use halo2_proofs::plonk::{Advice, Column, Error, Expression, Fixed, VirtualCells, Selector};
